    ...
    >
      <CardHeader className="space-y-2 pb-3">
        <div className="flex items-start justify-between gap-2">
          <CardTitle className="line-clamp-2 text-base font-semibold leading-tight sm:text-lg">{plan.title}</CardTitle>
        </div>
      ...
      <CardContent className="flex flex-1 flex-col justify-between space-y-3 text-xs text-muted-foreground sm:text-sm">
        ...
        <div className="mt-2 flex flex-wrap gap-2 text-[11px] font-medium sm:text-xs">
          <button type="button" className="text-primary hover:underline" onClick={() => onViewSessions(plan.id)}>
            View sessions
          </button>
          ...
          <span className="text-muted-foreground">·</span>
          <button type="button" className="text-primary hover:underline" onClick={() => onEdit(plan.id)}>
            Edit plan
          </button>
          <span className="text-muted-foreground">·</span>
          <button type="button" className="text-primary hover:underline" onClick={handleStatusClick}>
            {plan.status === "archived" ? "Unarchive" : "Archive"}
          </button>
          <span className="text-muted-foreground">·</span>
          <button type="button" className="text-destructive hover:underline" onClick={() => onDelete(plan.id)}>
            Delete
          </button>
        </div>
      </CardContent>
